...
pub fn draw_footer(app: &mut App, footer_area: Rect, frame: &mut Frame) {
    ...
    let [current_song_section, status_section, next_song_section] = footer_sections.areas(footer_inner);
    ...
    let current_song_info = if app.now_playing.is_empty() {
        ...
        Paragraph::new(Text::from(vec![
            ...
            Line::from(
                Span { content: ellipse_line(song.artist(), max_width).into(), style: Style::default().fg(Gray).add_modifier(Modifier::ITALIC) },
            ),
        ]))
    };
