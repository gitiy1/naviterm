...
async fn main() -> AppResult<()> {
        ...
        .build()
    {
        ...
        Err(e) => {
            println!(
                "Error loading configuration at HOME/.config/naviterm/config.ini: {}",
                e
            );
            exit(1);
        }
    ...
    if app.mode != AppConnectionMode::Offline {
        match app.test_connection().await {
            ...
            Err(e) => {
                ...
                warn!(
                    "Could not connect to server, starting offline! Error: {}",
                    e
                )
            }
        }
        ...
        match tui.events.next().await? {
            ...
            Event::Key(key_event) => {
                if let Err(e) = handle_key_events(key_event, &mut app, dbus_handler).await {
                    error!("Unmanaged error while processing the key event: {}", e)
                }
                if let Err(e) = tui.draw(&mut app) {
                    error!("Unmanaged error while drawing the UI: {}", e)
                }
            }
            Event::Resize(_, _) => {}
            Event::Dbus(dbus_event) => {
                if let Err(e) = handle_dbus_events(dbus_event, &mut app, dbus_handler).await {
                    error!("Unmanaged error while processing the dbus event: {}", e)
                }
            }
    ...
    if let Err(e) = tui.exit() {
        error!("Unmanaged error while exiting tui: {}", e);
    }
